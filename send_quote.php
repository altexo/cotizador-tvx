    ...
    { 
      ...
      if ($_POST["licencias"] == 1) 
      {
          ...
          if (isset($_POST['data']['select']['tvx_lic']) && ($_POST['data']['select']['tvx_cant'] > 0)) 
          {
              ...
              $subtotal = $round_act_tot + $total_mensual;
                $iva = $subtotal* 0.16;
               ...
               $r_iva_mensual = round($iva_mensual, 2, PHP_ROUND_HALF_UP);
                $total_mensual_mas_iva = $r_iva_mensual + $total_mensual;
          ...
          <!--<table><tr><td style="float: left; border: 0px; color: white; background-color: #ee6b1b;     margin-top: 100px; padding"><p>PAQUETE #'.$no.'</p></td></tr></table>-->
          <table><tr><td style="float: left; border: 0px;"><p> DETALLE PAGO INICIAL - ÚNICA VEZ</p></td></tr></table>
          ...
          <tr>
            <td style="float: left;">ACTIVACIÓN / CONFIGURACIÓN DE USUARIOS  Y GRUPOS</td>
            <td align="center">'.$cantidad.'</td>
            <td align="right">$ '.number_format($round_activ, 2, '.', ',').'</td>
            <td align="right">$ '.number_format($round_act_tot, 2, '.', ',').'</td>
          </tr>
          <tr>
            <td align="left" style="float: left; height: 100px;">
              '.$descripcion_paquete.'
          ...
          <tr>
            <td style="float: left; height: 100px;">
              '.$descripcion_paquete.'
          ...
          <tr>
            ...
            <td colspan="3" align="right"> Impuestos 16.00%</td>
            <td align="right">$ '.number_format($r_iva_mensual, 2, '.', ',').'</td>
          </tr>
          <tr style="background-color: #d0d0d0; border-top: 5px solid #656464;">
            <td colspan="3" align="right"> Total</td>
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////   
          if (isset($_POST['data']['select']['mctrl_lic']) && ($_POST['data']['select']['mctrl_cant'] > 0)) 
          {
            $no++;
              ...
              $total_mensual = $cantidad * $MOBICONTROL;
              $tarifa_uni = $MOBICONTROL;

              $total_activ = $activ * $cantidad;
              //$round_act_tot = round($total_activ, 2, PHP_ROUND_HALF_UP);
               ...
               $total_mensual = ($cantidad * $tarifa_uni);
                ...
                $descripcion_paquete = "<p>Licencia Mobicontrol (Administración dispositivos móviles y sus aplicaciones, contenido y seguridad. )</p>";
             ...
             <!-- <table><tr><td style="float: left; border: 0px; color: white; background-color: #ee6b1b;     margin-top: 100px; padding"><p>PAQUETE #'.$no.'</p></td></tr></table>-->
              ...
              <tr>
                ...
                <td align="right">$ '.number_format($total_activ, 2, '.', ',').'</td>
              </tr>
              <tr>
                <td align="left" style="float: left; height: 100px;">
                  '.$descripcion_paquete.'
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////          
        if (isset($_POST['data']['select']['evi_lic']) && ($_POST['data']['select']['evi_cant'] > 0)) 
        {
          $no++;
            ...
            $descripcion_paquete = "<p> Licencia Evidence Básico Radio PTT Ilimitado, Grupos Ilimitados, Usuarios Ilimitados.  Cobertura Nacional e Internacional, GPS, Consola de Despacho para rastreo de Unidades. </p>";
              $tabla_contenido .='
           <!--<table><tr><td style="float: left; border: 0px; color: white; background-color: #ee6b1b;     margin-top: 100px; padding"><p>PAQUETE #'.$no.'</p></td></tr></table>-->
          <table><tr><td style="float: left; border: 0px;"><p> DETALLE PAGO INICIAL - ÚNICA VEZ</p></td></tr></table>
          ...
          if (isset($_POST['data']['select']['tcall_lic']) && ($_POST['data']['select']['tcall_cant'] > 0)) 
          {
            $no++;
              ...
              $total = $cantidad * $TRUSTCALL;
              $tarifa_uni = $TRUSTCALL;
           ...
           $total_activ = $activ * $cantidad;
              //$round_act_tot = round($total_activ, 2, PHP_ROUND_HALF_UP);
               ...
               $total_mensual = ($cantidad * $tarifa_uni);
                ...
                $descripcion_paquete = "<p>Licencia TrustCall (Encripción de llamadas de Voz y mensajería de textos AES256bits)</p>";
...
file_put_contents($file_to_save, $pdf);
 ...
 $mail = new PHPMailer;
  ...
  $mail->Username = 'svt.fax@servitron.net';                 // SMTP username
  $mail->Password = 'GtF45Er';                           // SMTP password
  $mail->SMTPSecure = 'ssl';                            // Enable TLS encryption, `ssl` also accepted
  $mail->Port = 465;                                    // TCP port to connect to

  $mail->CharSet = 'UTF-8';
  $mail->setFrom('no-reply@teamvox.com', 'TeamVOX');
  $mail->addAddress($cli_mail, $cli_nombre);     // Add a recipient
  //$mail->addAddress('ellen@example.com');               // Name is optional
  $mail->addReplyTo('no-reply@example.com', 'Information');
  $mail->addCC('cc@example.com');
  $mail->addBCC('bcc@example.com');

  $mail->addAttachment('quotes/file.pdf');         // Add attachments
  $mail->addAttachment('/tmp/image.jpg', 'new.jpg');    // Optional name
  $mail->isHTML(true);                                  // Set email format to HTML
  $mail->Body = " ";
  $mail->Subject = 'Cotización #'.$folio.'';
    if(!$mail->send()) {

        echo  $mail->ErrorInfo;
    } else {
      ob_start(); 
   		header("refresh: 1; url = https://teamvox.com/cotizador/");
    	 echo '<script>alert("Su cotización a sido enviada correctamente")</script>';
     ;
    }
