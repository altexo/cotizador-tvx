...
$response[] = array('empty' => 0);
    ...
    $n_fecha = $myDateTime->format('d/m/Y');
        ...
        $img_name = basename($k . $_FILES["fileUpload"]["name"]);
		$uploadOk = 1;
		$imageFileType = pathinfo($target_file,PATHINFO_EXTENSION);
		// Check if image file is a actual image or fake image

		if (move_uploaded_file($_FILES["fileUpload"]["tmp_name"], $target_file)) 
		{       //Inicia Calculador de Licencias
			
			/*if ($_POST["ubiqo_smart"] == 1) {
				if (isset($_POST['data']['select']['cantidad_ubsmart'])) {
					$cantidad_ub = $_POST['data']['select']['cantidad_ubsmart'];
				}
				$plazo = $_POST['select']['plazo_ubsmart'];
				$costo_de_ub = $eq_ubiqo_smart * $currencyValue;
				

			}
			else*/ if ($_POST["licencias"] == 1) 
			{
